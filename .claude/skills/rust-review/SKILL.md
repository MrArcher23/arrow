---
description: Revisa la calidad del código Rust de arrow (el parser/lib en src/ y el backend Tauri en src-tauri/) aplicando clippy + rustfmt y un checklist de best-practices afinado al proyecto. Úsalo tras tocar src/lib.rs, src/main.rs o src-tauri/, o cuando quieras endurecer el código Rust. No reemplaza a /verify-parser (correctitud de datos); este mira la calidad del código.
...
- `src-tauri/` → el backend de escritorio.
...
- Backend Tauri: `(cd src-tauri && cargo clippy --release)` → **0 warnings**.
- Reporta cada warning con su `-->` archivo:línea. No los silencies con `#[allow]` sin justificarlo.

## 2. Formato con rustfmt
- `cargo fmt --check` en la raíz y en `src-tauri/`. Si hay diffs, muéstralos.
...
- **Sin `.unwrap()`/`.expect()` sobre datos externos** (disco, JSON, entorno). Permitido solo en
  invariantes del programa (p.ej. `generate_context!` de Tauri). Señala cada caso y clasifícalo.
- **Errores**: la CLI usa `anyhow::Result`; los comandos Tauri devuelven el struct directo (cualquier
  fallo de IO degrada a campos vacíos / `*_available:false`, nunca rompe la ventana).
- **La lógica del parser NO se duplica**: `src-tauri` y la CLI llaman a la misma lib (`arrow::`).
  Si ves lógica de parsing copiada, es un bug de diseño.
- **No romper el contrato JSON**: los structs `*Out` derivan `Serialize` con `rename_all="camelCase"`;
  un cambio de nombre/campo rompe la UI (`web/src/lib/types.ts`).
